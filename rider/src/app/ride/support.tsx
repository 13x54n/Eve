...
export default function SupportListScreen() {
  ...
  const [subject, setSubject] = useState(tripId ? "" : "");
  ...
  async function startTicket() {
    ...
    try {
      ...
      setBody("");
      router.push({ pathname: "/ride/support/[id]", params: { id: ticket.id } });
    } catch (error: unknown) {
    ...
    >
      ...
      {tickets.map((ticket) => (
        <Pressable
          key={ticket.id}
          style={styles.ticket}
          onPress={() => router.push({ pathname: "/ride/support/[id]", params: { id: ticket.id } })}
        >
