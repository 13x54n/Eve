    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: brand.canvas }]} edges={["top"]}>
      ...
      >
        <Pressable style={styles.modalOverlay} onPress={() => setShowReceiveQR(false)}>
          <View style={styles.modalContent} onStartShouldSetResponder={() => true}>
            ...
            <Text style={styles.qrAddress}>{wallet?.ethereumWallet?.substring(0, 6) + "..." + wallet?.ethereumWallet?.substring(wallet?.ethereumWallet.length - 4)}</Text>
      ...
      <View style={styles.topBar}>
        ...
        <Pressable onPress={() => setHidden((value) => !value)}>
          <Feather name={hidden ? "eye-off" : "eye"} size={20} color={Brand.text} />
        </Pressable>
      </View>
      <ScrollView
