...
1. Read [GETTING_STARTED.md](GETTING_STARTED.md) and run the six backend services locally (`cd backend && npm run dev`, or `docker compose up --build` for the same stack).
...
3. Do not add an HTTP gateway. Clients call auth `:4001`, ride `:4003`, payment `:4006`, notify `:4004`, and admin `:4005` directly.
