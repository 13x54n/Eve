...
export async function getDashboard(query: Record<string, unknown>) {
  ...
  const [
    ...
    prisma.ledgerEntry.aggregate({
      where: { type: "CHARGE", status: "COMPLETED", createdAt: { gte: today } },
      _sum: { amount: true },
    }),
    prisma.ledgerEntry.aggregate({
      where: { type: "REFUND", status: "COMPLETED", createdAt: { gte: today } },
      _sum: { amount: true },
    }),
    prisma.ledgerEntry.aggregate({
  ...
  const bookings = money(todayCharges._sum.amount);
  const refunds = money(todayRefunds._sum.amount);
...
function serializeDriver(
  ...
  return {
    ...
    commissionTier: driver.commissionTier,
    city: driver.city,
    lat: driver.latitude,
    lng: driver.longitude,
...
export async function getAnalytics(query: Record<string, unknown>) {
  ...
  const [byStatus, byCity, byType, totals, ratings, failures, incidents] =
    await Promise.all([
      ...
      prisma.trip.groupBy({
        ...
        where,
        _count: { _all: true },
        _sum: { fareTotal: true, commission: true },
      }),
      prisma.trip.groupBy({
        by: ["rideType"],
        where,
  ...
  return {
    ...
    cities: byCity.map((row) => ({
      ...
      commission: money(row._sum.commission),
    })),
    rideTypes: byType,
