...
Embedded Ethereum wallets sign RideEscrow txs on Arc Testnet. The contract address comes from payment (`GET /api/payment/config`), not Expo env. Fund the **embedded** address at [faucet.circle.com](https://faucet.circle.com). See [driver-wallet.md](driver-wallet.md).
