...
Connection: `REDIS_URL` (default `redis://127.0.0.1:6379`). Compose: `redis:7-alpine` in [`docker-compose.yml`](../docker-compose.yml). Production hosts must already have Redis reachable; deploy does not install it.
